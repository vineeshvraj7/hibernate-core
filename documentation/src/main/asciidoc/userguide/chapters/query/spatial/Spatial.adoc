...
DB2::
The `DB2SpatialDialect` supports the spatial extensions of the DB2 database. The dialect has been tested with DB2 LUW 11.1
[NOTE]
====
In DB2 _transfrom groups_ are used to transfer geometries between the database and client applications. Currently Hibernate
Spatial only supports the ST_WellknownText transfer group.
====

This dialect can be configured using the Hibernate property:  +

`hibernate.spatial.db2.srid`:::
the EPSG code to use when deserializing geometries from the database.

.Why the `hibernate.spatial.db2.srid` property?
[NOTE]
====
The DB2 _transfrom groups_ have the property that when transferring geometries to the database the SRID is set to 0, effectively
dropping the SRID information from Geometry. The ST_WellknownText transfer group (see above) has the additonal property that its output Wellknow Text
representation contains no SRID information. +
To provide at least a measure of control we have
provided the `hibernate.spatial.db.srid` property. When reading a geometry from DB2, its SRID will be set to the value of this property. 
====
