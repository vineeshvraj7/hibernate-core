...
[NOTE]
====
The Wellknowntext (WKT) representation used to transfer geometry information to and from DB2 does not include the srid information.

The srid for use when retrieving geometries by specifying a value in the hibernate.properties file like:
....
hibernate.spatial.db2.srid = 4326
....
for example when using the WGS84 coordinate system.

The srid for use when storing geometries can be specified in the Java class representing the table with a spatial column by including lines like:
[%hardbreaks]
    import org.hibernate.annotations.ColumnTransformer;
	@ColumnTransformer(write="db2gse.st_geometry(cast( ? as clob(1M)),4326)")
