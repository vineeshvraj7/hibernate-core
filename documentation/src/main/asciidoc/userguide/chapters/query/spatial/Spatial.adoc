...
The easiest ways to do this are to:

* Copy these statements into a SQL script in Eclipse with Data Tools Platform (DTP) or IBM Data Studio
and execute them there.
* or
* Copy these statements into a file such as ewkt.sql and execute them in a DB2 command window with a command like 'db2 -tvf ewkt.sql'.
